...
type ParentCat = {
  name:     string
  slug:     string
  oldSlugs: string[]   // existing flat categories to absorb
...
const TREE: ParentCat[] = [
  ...
  },
  {
    ...
    subCats: [
      ...
      {
        name: 'General Computer Supplies',
        slug: 'ch--general',
        keywords: [],
      },
    ],
...
// These fix products that are in the WRONG parent category (e.g. ladders in
// 'office-machine'). Checked BEFORE the per-parent sub-cat matching.
...
const OVERRIDES: Override[] = [
  ...
  // Punch cards in 'office-stationery' belong in Office Machine
...
function classify(productName: string, currentParentSlug: string): { parentSlug: string; subSlug: string } {
  // Step 1: High-priority overrides (fix misclassifications)
  ...
  // Step 2: Find the right parent from current category
  const parent = TREE.find(p => p.slug === currentParentSlug || p.oldSlugs.includes(currentParentSlug))

  if (parent) {
    // Step 3: Find best sub-category by keyword within this parent
  ...
  // Step 5: Default to Office Stationery > General
